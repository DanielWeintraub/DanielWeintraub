...
def syncJob(name, project, dest):
    ...
    r = requests.get(f'https://{config["host"]["snd"]}/api/14/project/{project}/jobs/export',
                     headers=snd_headers,
                     params=myparams)
    ...
    dest_headers = {'X-Rundeck-Auth-Token': config['creds'][dest],
                    'Content-Type': 'application/yaml',
                    "Accept": 'application/json'}
    myparams = {'dupeOption': 'update'}
    r = requests.post(f'https://{config["host"][dest]}/api/14/project/{project}/jobs/import',
                      headers=dest_headers,
                      params=myparams,
                      data=yaml.dump([jobDef]))
